...
public class AuthService {
    ...
    public boolean register(String firstName, String lastName, String email, String password, UserRole role) {
        ...
        if (loggedInUser.getUserRole() == UserRole.DIRECTOR) {
            return authService.register(firstName, lastName, email, password, role);
        }
        else if (loggedInUser.getUserRole() == UserRole.RESPONSIBLE) {
            if (role == UserRole.AGENT) {
                return authService.register(firstName, lastName, email, password, UserRole.AGENT);
            } else {
                System.out.println("Responsible can only create Agents.");
                return false;
            }
        }

        System.out.println("This role cannot register new users.");
        return false;
    }
